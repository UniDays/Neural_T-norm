...
class QueryExecutor(nn.Module, core.Configurable):
    ...
    def __init__(self, model, num_node, t_norm_hidden_dims,logic="product", dropout_ratio=0.25, num_mlp_layer=2):
        ...
        # t_norm learnable
        self.t_norm = T_Norm_NN(num_node, t_norm_hidden_dims)
    ...
    def execute(self, graph, query, all_loss=None, metric=None):
        ...
        # instruction pointer
        # recording the operation has been computed of each query
        ...
        while not op.is_stop().all():
            ...
            # sinc projection is the most expensive and we want to maximize the parallelism
            ...
            # update the instruction of query where has been computed
            op = query[all_sample, self.IP]
    ...
    def apply_operand(self, mask, h_index, num_node):
        # get anchor entity for each query
        h_prob = functional.one_hot(h_index, num_node)
        # push the head entities into stack
...
@R.register("model.T_Norm")
class T_Norm_NN (nn.Module):
    def __init__(self, num_node, hidden_dims):
        super().__init__()
        # neural bias of t_norm
        self.hidden1 = nn.Linear(num_node * 2, hidden_dims[0])
        self.hidden2 = nn.Linear(hidden_dims[0], hidden_dims[1])
        self.hidden3 = nn.Linear(hidden_dims[2], num_node)

        # conbine the output of ordinary t_norm and nerual bias
        self.ordnary_t_norm_weight = torch.randn(1, requires_grad=True, device='cuda')
        self.nn_t_norm_weight = torch.randn(1, requires_grad=True, device='cuda')
    
    def forward(self, x, y):
        nb_1 = torch.cat([x, y], dim = -1)
        nb_2 = torch.cat([y, x], dim = -1)

        nb_1 = F.relu(self.hidden1(nb_1))
        nb_1 = F.relu(self.hidden2(nb_1))
        nb_1 = self.hidden3(nb_1)

        nb_2 = F.relu(self.hidden1(nb_2))
        nb_2 = F.relu(self.hidden2(nb_2))
        nb_2 = self.hidden3(nb_2)

        nb = F.sigmoid(nb_1 + nb_2)

        #combine layer
        output = F.sigmoid(self.ordnary_t_norm_weight * (x * y) + self.nn_t_norm_weight * nb)
